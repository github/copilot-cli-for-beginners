...
> **Copilot이 GitHub 이슈를 읽고, 데이터베이스를 확인하고, PR까지 만들 수 있다면... 그것도 모두 터미널에서 가능하다면 어떨까요?**

지금까지 Copilot은 여러분이 직접 제공한 것만 다룰 수 있었습니다. `@`로 참조한 파일, 대화 기록, 그리고 자체 학습 데이터가 그것이죠. 그런데 만약 Copilot이 스스로 GitHub 저장소를 확인하거나, 프로젝트 파일을 둘러보거나, 라이브러리의 최신 문서를 찾아볼 수 있다면 어떨까요?
...
> 💡 **이미 MCP에 익숙하신가요?** [퀵 스타트로 바로 이동](#-use-the-built-in-github-mcp)해서 정상 동작을 확인하고 서버를 구성해 보세요.
...
MCP 서버를 브라우저 확장 프로그램이라고 생각해 보세요. 브라우저 자체로도 웹 페이지를 보여줄 수 있지만, 확장 프로그램은 브라우저를 추가 서비스에 연결해 줍니다:
...
GitHub MCP 서버는 기본으로 포함되어 있습니다. 다음을 시도해 보세요:
...
> 💡 **GitHub 서버만 보이나요?** 정상입니다! 추가 MCP 서버를 아직 추가하지 않았다면 GitHub만 표시됩니다. 다음 섹션에서 더 추가해 보겠습니다.

> 📚 **모든 MCP 관리 명령을 보고 싶으신가요?** 채팅 안에서 `/mcp` 슬래시 명령으로 또는 터미널에서 직접 `copilot mcp`로 서버를 관리할 수 있습니다. 이 챕터 끝에 있는 [전체 명령 레퍼런스](#-additional-mcp-commands)를 참고하세요.
...
> 📚 **공식 문서**: GitHub Copilot에서 MCP가 어떻게 동작하는지 더 깊이 알아보려면 [About MCP](https://docs.github.com/copilot/concepts/context/mcp)를 참고하세요.
...
이제 MCP가 동작하는 모습을 보았으니, 추가 서버를 설정해 봅시다. 서버를 추가하는 방법은 두 가지입니다: **내장 레지스트리에서 추가**(가장 쉬움 — CLI에서 가이드 설정)하거나 **구성 파일을 수동으로 편집**(더 유연함)하는 것입니다. 어떤 방법을 선택할지 모르겠다면 레지스트리 옵션부터 시작해 보세요.
...
> ⚠️ **참고**: `.vscode/mcp.json`은 더 이상 MCP 구성 소스로 지원되지 않습니다. 기존 `.vscode/mcp.json`이 있다면 프로젝트 루트의 `.mcp.json`으로 마이그레이션하세요. CLI는 오래된 구성 파일을 감지하면 마이그레이션 힌트를 표시합니다.
...
<details>
<summary>💡 <strong>JSON이 처음이신가요?</strong> 각 필드가 무엇을 의미하는지 보려면 클릭하세요</summary>
...
- 파일은 유효한 JSON이어야 합니다 (확신이 없다면 [JSON 검증기](https://jsonlint.com/)를 사용하세요)
...
GitHub MCP 서버는 내장되어 있어서 별도 설정이 필요 없습니다. 아래는 추가로 더할 수 있는 서버입니다. **관심 있는 것을 고르거나 순서대로 진행해 보세요.**

| 하고 싶은 일... | 이동 |
|---|---|
| Copilot이 내 프로젝트 파일을 탐색하게 하기 | [Filesystem 서버](#filesystem-server) |
| 최신 라이브러리 문서 가져오기 | [Context7 서버](#context7-server-documentation) |
| 선택적 추가 기능 살펴보기 (커스텀 서버, web_fetch) | [기본을 넘어서](#beyond-the-basics) |
...
이를 `~/.copilot/mcp-config.json`에 추가하고 Copilot을 재시작하세요.
...
Copilot을 자신의 API, 데이터베이스 또는 내부 도구에 연결하고 싶으신가요? Python으로 커스텀 MCP 서버를 구축할 수 있습니다. 사전 구축된 서버(GitHub, filesystem, Context7)가 대부분의 사용 사례를 다루므로 이는 완전히 선택 사항입니다.

📖 book app을 예시로 한 전체 워크스루는 [커스텀 MCP 서버 가이드](mcp-custom-server.md)를 참고하세요.

📚 더 많은 배경 지식은 [MCP for Beginners 코스](https://github.com/microsoft/mcp-for-beginners)를 참고하세요.
...
전역 접근을 위해 `~/.copilot/mcp-config.json`으로 저장하거나, 프로젝트별 구성을 위해 프로젝트 루트의 `.mcp.json`으로 저장하세요.
...
**탐색할 서버를 고르거나 순서대로 진행해 보세요.**

| 시도해 보고 싶은 것... | 이동 |
|---|---|
| GitHub 저장소, 이슈, PR | [GitHub 서버](#github-server-built-in) |
| 프로젝트 파일 탐색 | [Filesystem 서버 사용](#filesystem-server-usage) |
| 라이브러리 문서 조회 | [Context7 서버 사용](#context7-server-usage) |
| 커스텀 서버, Microsoft Learn MCP 및 web_fetch 사용 | [기본을 넘어서 - 사용](#beyond-the-basics-usage) |
...
> 💡 **동작하지 않나요?** `/login`을 실행하여 GitHub로 다시 인증하세요.
...
- **로컬 dev container (Docker)**: 컨테이너가 시작된 후 `gh auth login`을 실행한 다음 Copilot을 재시작하세요.
...
> 💡 **자신의 포크에서 작업 중이신가요?** 이 코스 저장소를 포크했다면 이슈와 풀 리퀘스트를 만드는 것 같은 쓰기 작업도 시도해 볼 수 있습니다. 아래 연습에서 함께 실습해 봅니다.

> ⚠️ **결과가 보이지 않나요?** GitHub MCP는 로컬 파일이 아니라 저장소의 원격(github.com)을 대상으로 동작합니다. 저장소에 원격이 있는지 확인하세요: `git remote -v`로 확인할 수 있습니다.
...
**Microsoft Learn MCP**: [Microsoft Learn MCP 서버](#microsoft-learn-mcp-server)를 설치했다면 공식 Microsoft 문서를 직접 조회할 수 있습니다:
...
아래 각 예시는 독립적입니다. **관심 있는 것을 골라 보거나 모두 읽어 보세요.**

| 보고 싶은 것... | 이동 |
|---|---|
| 여러 서버가 함께 동작하는 모습 | [다중 서버 탐색](#multi-server-exploration) |
| 한 세션에서 이슈에서 PR까지 진행 | [이슈에서 PR로 워크플로](#issue-to-pr-workflow) |
| 빠른 프로젝트 상태 점검 | [상태 대시보드](#health-dashboard) |
...
> 💡 **지금 시도해 볼 수 없어도 걱정하지 마세요.** 읽기 전용 클론에 있다면 과제에서 연습하게 됩니다. 지금은 흐름을 이해하기 위해 그냥 읽어 보세요.
...
이제 여러분 차례입니다! 다음 연습을 완료하여 book app 프로젝트에서 MCP 서버를 사용하는 방법을 연습해 보세요.
...
GitHub 서버가 활성화된 상태로 표시되어야 합니다. 그렇지 않다면 `/login`을 실행하여 인증하세요.

---

### 연습 2: Filesystem MCP로 Book App 탐색하기

filesystem 서버를 구성했다면 이를 사용해 book app을 탐색해 보세요:
...
> 💡 **filesystem MCP를 아직 구성하지 않으셨나요?** 위 [전체 구성](#complete-configuration-file) 섹션에서 구성 파일을 만드세요. 그런 다음 Copilot을 재시작하세요.

---

### 연습 3: GitHub MCP로 저장소 이력 조회하기

내장 GitHub MCP를 사용하여 이 코스 저장소를 탐색해 보세요:
...
> ⚠️ **Codespace에 있나요?** 자동으로 동작합니다. 인증이 상속됩니다. 로컬 클론에서는 `gh auth status`로 로그인되어 있는지 확인하세요.

---

### 연습 4: 여러 MCP 서버 결합하기

이제 단일 세션에서 filesystem과 GitHub MCP를 결합해 보세요:
...
book app 프로젝트에서 MCP 서버를 함께 사용하는 연습을 합니다. 단일 Copilot 세션에서 다음 단계를 완료하세요:

1. **MCP 동작 확인**: `/mcp show`를 실행하고 최소한 GitHub 서버가 활성화되어 있는지 확인하세요
2. **filesystem MCP 설정**(아직 안 했다면): filesystem 서버 구성으로 `~/.copilot/mcp-config.json`을 만드세요
3. **코드 탐색**: Copilot에게 filesystem 서버를 사용하도록 요청하세요:
   - `samples/book-app-project/books.py`의 모든 함수 나열
   - `samples/book-app-project/utils.py`에서 타입 힌트가 없는 함수 확인
   - `samples/book-app-project/data.json`을 읽고 데이터 품질 문제 식별 (힌트: 마지막 항목을 보세요)
4. **저장소 활동 확인**: Copilot에게 GitHub MCP를 사용하도록 요청하세요:
...
위 [전체 구성](#complete-configuration-file) 섹션의 JSON을 사용하여 `~/.copilot/mcp-config.json`으로 저장하세요.
...
**MCP가 동작하지 않는다면:** 구성 파일을 편집한 후 Copilot을 재시작하세요.

</details>

### 보너스 챌린지: 커스텀 MCP 서버 구축하기

더 깊이 들어갈 준비가 되셨나요? [커스텀 MCP 서버 가이드](mcp-custom-server.md)를 따라 어떤 API에든 연결되는 자신만의 MCP 서버를 Python으로 구축해 보세요.
...
| 실수 | 결과 | 해결 |
|---------|--------------|-----|
| GitHub MCP가 내장임을 모름 | 수동으로 설치/구성하려 함 | GitHub MCP는 기본 포함입니다. 그냥 시도해 보세요: "List the recent commits in this repo" |
| 잘못된 위치에서 구성 파일을 찾음 | MCP 설정을 찾거나 편집할 수 없음 | 사용자 수준 구성은 `~/.copilot/mcp-config.json`에, 프로젝트 수준은 프로젝트 루트의 `.mcp.json`에 있습니다 |
| 구성 파일의 잘못된 JSON | MCP 서버 로드 실패 | `/mcp show`로 구성을 확인하세요. JSON 구문을 검증하세요 |
| MCP 서버 인증을 잊음 | "Authentication failed" 오류 | 일부 MCP는 별도 인증이 필요합니다. 각 서버의 요구사항을 확인하세요 |

### 문제 해결

**"MCP server not found"** - 다음을 확인하세요:
1. npm 패키지가 존재하는지: `npm view @modelcontextprotocol/server-github`
2. 구성이 유효한 JSON인지
3. 서버 이름이 구성과 일치하는지

`/mcp show`로 현재 구성을 확인하세요.

**"GitHub authentication failed"** - 내장 GitHub MCP는 `/login` 자격 증명을 사용합니다. 시도해 보세요:
...
이는 GitHub로 다시 인증합니다. 문제가 지속되면 GitHub 계정이 접근하려는 저장소에 필요한 권한이 있는지 확인하세요.

**"MCP server failed to start"** - 서버 로그를 확인하세요:
```bash
# Run the server command manually to see errors
npx -y @modelcontextprotocol/server-github
```

**MCP 도구가 사용 불가** - 서버가 활성화되어 있는지 확인하세요:
```bash
copilot
...
서버가 비활성화된 경우 다시 활성화하는 방법은 아래의 [추가 `/mcp` 명령](#-additional-mcp-commands)을 참고하세요.
...
> 💡 **언제 어느 것을 사용해야 하나요?** 이미 채팅 세션에 있을 때는 `/mcp` 슬래시 명령을 사용하세요. 세션을 시작하기 전에 서버 설정을 빠르게 확인하거나 변경하고 싶을 때는 터미널에서 `copilot mcp`를 사용하세요.
...
5. **두 가지 방법으로 서버 관리**: 채팅 안에서 `/mcp` 슬래시 명령을 사용하거나, 터미널에서 `copilot mcp`를 사용하세요
6. **커스텀 서버**는 어떤 API에든 연결할 수 있게 해줍니다 (선택, 부록 가이드에서 다룸)

> 📋 **빠른 참조**: 명령과 단축키의 전체 목록은 [GitHub Copilot CLI 명령 레퍼런스](https://docs.github.com/en/copilot/reference/cli-command-reference)를 참고하세요.
