...
> **AI가 버그를 즉시 발견하고, 혼란스러운 코드를 설명하고, 동작하는 스크립트를 생성하는 모습을 보세요. 그런 다음 GitHub Copilot CLI를 사용하는 세 가지 방식을 배워 보세요.**
...
> ⚠️ **사전 요건**: 아직 [**챕터 00: 빠른 시작**](../00-quick-start/README.md)을 완료하지 않았다면 먼저 완료하세요. 아래 데모를 실행하기 전에 GitHub Copilot CLI가 설치되고 인증되어 있어야 합니다.
...
인상적인 데모에 들어가기 전에 지금 바로 시도해 볼 수 있는 간단한 프롬프트로 시작해 봅시다. **코드 저장소가 필요 없습니다!** 터미널을 열고 Copilot CLI를 시작하세요:

```bash
copilot
```

초보자 친화적인 프롬프트를 시도해 보세요:
...
얼마나 자연스러운지 느껴 보세요. 동료에게 묻는 것처럼 질문만 하면 됩니다. 탐색이 끝나면 `/exit`를 입력하여 세션을 종료하세요.
...
> 💡 **예제 출력에 대하여**: 강의 전반에 걸쳐 표시되는 샘플 출력은 예시입니다. Copilot CLI의 응답은 매번 다르므로 단어, 형식, 세부 사항이 다를 수 있습니다. 정확한 텍스트보다는 반환되는 *정보의 유형*에 집중하세요.

### 데모 1: 초 단위 코드 리뷰

강의에는 의도적인 코드 품질 문제가 있는 샘플 파일이 포함되어 있습니다. 로컬 머신에서 작업 중이며 아직 저장소를 클론하지 않았다면 아래 `git clone` 명령어를 실행하고 `copilot-cli-for-beginners` 폴더로 이동한 다음 `copilot` 명령어를 실행하세요.

```bash
# 로컬에서 작업 중이며 아직 클론하지 않았다면 저장소를 클론하세요
...
대화형 Copilot CLI 세션 안에서 다음을 실행하세요:

```
> Review @samples/book-app-project/book_app.py for code quality issues and suggest improvements
```

> 💡 **`@` 기호는 무엇에 사용되나요?** `@` 기호는 Copilot CLI에게 파일을 읽으라고 지시합니다. 챕터 02에서 자세히 배울 예정입니다. 지금은 명령어를 그대로 복사하세요.
...
코드를 보면서 이게 무엇을 하는지 의아해한 적이 있나요? Copilot CLI 세션에서 이것을 시도해 보세요:
...
그렇지 않으면 15분을 구글링에 쓸 함수가 필요하신가요? 세션에서 계속:
...
GitHub Copilot CLI를 사용하는 것을 외식하는 것에 비유해 보세요. 식당 계획부터 주문까지, 상황마다 다른 접근이 필요합니다:
...
*작업에 따라 모드를 선택하세요: 계획이 먼저 필요할 때는 Plan, 대화형 협업에는 Interactive, 빠른 단발성 결과에는 Programmatic*

### 어떤 모드로 시작해야 할까요?

**Interactive 모드로 시작하세요.**
- 실험하고 후속 질문을 할 수 있습니다
- 대화를 통해 컨텍스트가 자연스럽게 쌓입니다
- 실수를 `/clear`로 쉽게 수정할 수 있습니다

익숙해지면 다음을 시도해 보세요:
- **Programmatic 모드** (`copilot -p "<프롬프트>"`) - 빠른 일회성 질문에
...
지금까지 보신 것처럼 자연스럽게 타이핑할 수 있는 프롬프트가 표시됩니다. 사용 가능한 명령어에 대한 도움말을 보려면 다음을 입력하세요:
...
각 프롬프트가 이전 답변을 기반으로 한다는 점을 주목하세요. 매번 처음부터 시작하는 것이 아니라 대화를 나누고 있습니다.
...
Plan 모드는 코드를 작성하기 전에 단계별 계획을 세우는 데 도움을 줍니다. `/plan` 명령어를 사용하거나 **Shift+Tab**을 눌러 Plan 모드로 전환하세요:
...
상호작용이 필요 없는 일회성 명령어에는 `-p` 플래그를 사용하세요:
...
```
> ⚠️ **`--allow-all`에 대하여**: 이 플래그는 모든 권한 프롬프트를 건너뛰어 Copilot CLI가 파일 읽기, 명령어 실행, URL 접근을 먼저 묻지 않고 수행하게 합니다. 대화형 세션이 없어 작업을 승인할 수 없는 programmatic 모드(`-p`)에서는 필요합니다. 직접 작성한 프롬프트와 신뢰할 수 있는 디렉터리에서만 `--allow-all`을 사용하세요. 신뢰할 수 없는 입력이나 민감한 디렉터리에서는 절대 사용하지 마세요.
...
> 📚 **공식 문서**: 전체 명령어 및 플래그 목록은 [CLI 명령어 참조](https://docs.github.com/copilot/reference/cli-command-reference)를 참조하세요.
...
> ⚠️ **주의해서 사용하세요**: `/allow-all`과 `/yolo`는 확인 프롬프트를 건너뜁니다. 신뢰할 수 있는 프로젝트에는 좋지만 신뢰할 수 없는 코드에는 주의하세요.
...
Copilot CLI는 OpenAI, Anthropic, Google 등의 여러 AI 모델을 지원합니다. 사용 가능한 모델은 구독 수준과 지역에 따라 다릅니다. `/model`을 사용하여 옵션을 확인하고 전환하세요:

```bash
copilot
> /model

# 사용 가능한 모델을 표시하고 선택할 수 있습니다. Sonnet 4.5를 선택하세요.
```

> 💡 **팁**: 일부 모델은 다른 것보다 더 많은 "프리미엄 요청"을 소비합니다. **1x**로 표시된 모델(Claude Sonnet 4.5 등)은 훌륭한 기본값입니다. 유능하고 효율적입니다. 배수가 높은 모델은 프리미엄 요청 할당량을 더 빠르게 사용하므로 정말 필요할 때를 위해 아껴 두세요.

> 💡 **어떤 모델을 선택해야 할지 모르겠나요?** 모델 선택기에서 **`Auto`**를 선택하면 Copilot이 각 세션에 가장 적합한 모델을 자동으로 선택합니다. 시작 단계이고 모델 선택에 대해 고민하고 싶지 않다면 훌륭한 기본값입니다.
...
Copilot을 시작하고 후속 프롬프트를 사용하여 도서 앱을 반복적으로 개선하세요:
...
`/plan`을 사용하여 Copilot CLI가 코드를 작성하기 전에 구현을 계획하게 하세요:
...
`-p` 플래그를 사용하면 대화형 모드에 들어가지 않고 터미널에서 직접 Copilot CLI를 실행할 수 있습니다. 다음 스크립트를 저장소 루트에서 터미널(Copilot 안이 아닌)에 복사하여 도서 앱의 모든 Python 파일을 검토하세요.

```bash
# 도서 앱의 모든 Python 파일 검토
...
데모를 완료한 후 이러한 변형을 시도해 보세요:

1. **Interactive 챌린지**: `copilot`을 시작하고 도서 앱을 탐색하세요. `@samples/book-app-project/books.py`에 대해 묻고 연속으로 3번 개선을 요청하세요.

2. **Plan 모드 챌린지**: `/plan Add rating and review features to the book app`을 실행하세요. 계획을 꼼꼼히 읽어 보세요. 이해가 되나요?

3. **Programmatic 챌린지**: `copilot --allow-all -p "List all functions in @samples/book-app-project/book_app.py and describe what each does"`를 실행하세요. 처음 시도에 잘 됐나요?
...
Copilot CLI가 링크를 표시하고 QR 코드에 접근할 수 있게 합니다. 휴대폰이나 데스크톱 브라우저 탭에서 링크를 열어 실시간으로 세션을 확인하고, 후속 프롬프트를 전송하고, 계획을 검토하고, 원격으로 에이전트를 조종하세요. 세션은 사용자별이므로 자신의 Copilot CLI 세션에만 접근할 수 있습니다.
...
실습 예제는 `book_app.py` 검토 및 리팩토링에 집중했습니다. 이제 다른 파일인 `utils.py`에서 동일한 기술을 연습해 보세요:
...
6. 컨텍스트가 프롬프트 간에 유지되는 방식을 관찰하세요. 각 개선이 이전 것을 기반으로 합니다
7. `/exit`로 종료
...
**흔한 문제:**
- Copilot CLI가 명확화 질문을 하면 자연스럽게 답변하세요
- 컨텍스트가 유지되므로 각 프롬프트가 이전 것을 기반으로 합니다
- 다시 시작하고 싶으면 `/clear`를 사용하세요

</details>

### 보너스 챌린지: 모드 비교하기

예제에서는 검색 기능에 `/plan`을, 배치 검토에 `-p`를 사용했습니다. 이제 새로운 작업인 `BookCollection` 클래스에 `list_by_year()` 메서드 추가를 세 가지 모드 모두로 시도해 보세요:
...
**성찰**: 어떤 모드가 가장 자연스럽게 느껴졌나요? 각 모드를 언제 사용하겠습니까?
...
| `$`나 `!`가 포함된 프롬프트에 따옴표 없이 사용 | 쉘이 Copilot CLI가 보기 전에 특수 문자를 해석 | 프롬프트를 따옴표로 감싸세요: `copilot -p "What does $HOME mean?"` |
| Esc를 한 번 눌러 실행 중인 작업 취소 | 단일 Esc는 더 이상 진행 중인 작업을 취소하지 않음(실수 방지) | Copilot CLI가 처리 중일 때 취소하려면 **Esc를 두 번** 누르세요 |

### 문제 해결

**"Model not available"** - 구독에 모든 모델이 포함되지 않을 수 있습니다. `/model`을 사용하여 사용 가능한 것을 확인하세요.

**"Context too long"** - 대화가 전체 컨텍스트 창을 사용했습니다. `/clear`를 사용하여 리셋하거나 새 세션을 시작하세요.

**"Rate limit exceeded"** - 몇 분 기다렸다가 다시 시도하세요. 딜레이를 포함한 배치 작업에는 programmatic 모드를 사용하는 것을 고려하세요.
...
2. **Plan 모드**는 일반적으로 더 복잡한 작업에 사용됩니다. 구현 전에 검토하세요.
