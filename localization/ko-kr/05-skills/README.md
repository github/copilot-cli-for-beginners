...
> **만약 Copilot이 매번 설명할 필요 없이 여러분 팀의 모범 사례를 자동으로 적용해 준다면 어떨까요?**
...
1. **이미 사용 가능한 스킬 확인하기:**
   ...
   > 💡 **기본 내장 스킬**: Copilot CLI에는 처음부터 사용할 수 있는 스킬이 미리 설치되어 있습니다. 예를 들어 `customizing-copilot-cloud-agents-environment` 스킬은 Copilot 클라우드 에이전트의 환경을 사용자 지정하는 가이드를 제공합니다. 이런 스킬은 별도로 만들거나 설치할 필요가 없습니다. `/skills list`를 실행해 어떤 스킬이 있는지 확인해 보세요.

2. **실제 스킬 파일 살펴보기:** 제공된 [code-checklist SKILL.md](../../../.github/skills/code-checklist/SKILL.md)를 보고 패턴을 확인해 보세요. YAML frontmatter와 마크다운 지시문으로만 구성되어 있습니다.
...
> 🧰 **바로 사용 가능한 템플릿**: 복사해 붙여 넣어 바로 시도해 볼 수 있는 간단한 스킬은 [.github/skills](../../../.github/skills/) 폴더에서 확인하세요.
...
> 💡 **팁**: 스킬 슬래시 명령은 문장에서 가장 자연스러운 곳에 두세요. 메시지의 시작, 중간, 끝 어디든 좋습니다.
...
> *MCP는 아직 걱정하지 마세요. [Chapter 06](../06-mcp-servers/)에서 다룹니다. 스킬이 전체 그림에 어떻게 들어맞는지 보여 주기 위해 여기에 함께 소개합니다.*
...
> 📚 **자세히 알아보기**: 스킬 형식과 모범 사례에 대한 전체 레퍼런스는 공식 [About Agent Skills](https://docs.github.com/copilot/concepts/agents/about-agent-skills) 문서를 참고하세요.
...
*자연스럽게 질문하세요. Copilot이 여러분의 프롬프트를 적절한 스킬에 매칭해 자동으로 적용합니다.*
...
여러분의 팀에 10가지 항목의 PR 체크리스트가 있다고 가정해 봅시다. 스킬이 없다면 모든 개발자가 10가지를 다 기억해야 하고, 누군가는 늘 한두 개를 빠뜨립니다. `pr-review` 스킬이 있으면 팀 전체가 일관된 리뷰를 받게 됩니다:
...
OWASP Top 10 취약점을 점검하는 보안 감사 스킬을 만들어 봅시다:
...
로컬 플러그인 카탈로그를 최신 상태로 유지하려면 다음을 실행해 새로 고치세요:
...
> ⚠️ **설치 전 확인하세요**: 스킬을 설치하기 전에는 항상 `SKILL.md`를 읽어 보세요. 스킬은 Copilot의 동작을 제어하기 때문에, 악의적인 스킬은 위험한 명령을 실행하거나 예기치 않은 방식으로 코드를 수정하도록 지시할 수 있습니다.
...
서로 다른 패턴을 보여 주는 스킬 두 개를 더 만들어 봅니다. 위의 "첫 번째 스킬 만들기"에서 사용한 `mkdir` + `cat` 워크플로를 따르거나, 스킬을 적절한 위치에 복사해 붙여 넣으세요. 더 많은 예제는 [.github/skills](../../../.github/skills)에 있습니다.
...
1. **스킬 만들기 챌린지**: 다음 3가지 항목을 점검하는 `quick-review` 스킬을 만들어 보세요:
   - bare except 절
   - 누락된 타입 힌트
   - 명확하지 않은 변수 이름

   "Do a quick review of books.py"라고 물어 테스트해 보세요.

2. **스킬 비교**: 자세한 보안 리뷰 프롬프트를 직접 작성하는 데 걸리는 시간을 측정해 보세요. 그런 다음 "Check for security issues in this file"이라고만 묻고 security-audit 스킬이 자동으로 불러와지도록 해 보세요. 스킬이 시간을 얼마나 절약해 주었나요?

3. **팀 스킬 챌린지**: 여러분 팀의 코드 리뷰 체크리스트를 떠올려 보세요. 그것을 스킬로 인코딩할 수 있을까요? 스킬이 항상 점검해야 할 3가지를 적어 보세요.
...
1. 현재 스킬 목록을 확인하세요: Copilot을 실행한 뒤 `/skills list`를 입력합니다. 프로젝트 스킬은 `ls .github/skills/`로, 개인 스킬은 `ls ~/.copilot/skills/`로도 확인할 수 있습니다.
...
4. 스킬을 테스트하세요: `@samples/book-app-project/data.json Summarize the books in this collection`
5. `/skills list`로 스킬이 자동 트리거되는지 확인하세요.
6. `/book-summary Summarize the books in this collection`으로 직접 호출도 시도해 보세요.
...
1. 일관된 형식의 컨벤셔널 커밋 메시지를 생성하는 `commit-message` 스킬을 만드세요.
2. 변경 사항을 스테이징한 뒤 "Generate a commit message for my staged changes"라고 물어 테스트해 보세요.
3. 스킬을 문서화하고 `copilot-skill` 토픽을 달아 GitHub에 공유하세요.
...
1. **description 확인**: 여러분이 묻는 방식과 일치하나요?
   ```markdown
   # Bad: Too vague
...
3. **있을 때와 없을 때 비교**: 같은 프롬프트를 `--no-custom-instructions`와 함께 실행해 차이를 확인해 보세요:
   ...
   ```
4. **특정 검사 항목 확인**: 스킬이 구체적인 검사("functions over 50 lines" 등)를 포함한다면 출력에 그것이 등장하는지 확인하세요.
...
> 📋 **빠른 참조**: 명령과 단축키 전체 목록은 [GitHub Copilot CLI command reference](https://docs.github.com/en/copilot/reference/cli-command-reference)를 확인하세요.
