...
title: !translate Construyendo un servidor MCP personalizado
description: !translate Crea un servidor MCP personalizado sencillo en Python para conectar GitHub Copilot CLI con tus propias APIs.
...
¿Quieres conectar Copilot a tus propias APIs? Aquí tienes cómo construir un servidor MCP sencillo en Python que busca información de libros, relacionándose con la aplicación de libros que has estado usando a lo largo de este curso.
...
> 💡 **¿Por qué los decoradores?** El decorador `@mcp.tool()` es todo lo que necesitas. El SDK de MCP lee automáticamente el nombre de tu función, las anotaciones de tipo y la docstring para generar el esquema de la herramienta. ¡No se necesita un esquema JSON manual!
...
1. **Añadir más herramientas** - Cada `@mcp.tool()` función se convierte en una herramienta que Copilot puede invocar
...
3. **Añadir autenticación** - Gestiona claves de API y tokens de forma segura
