...
¿Quieres conectar Copilot a tus propias APIs? Aquí se explica cómo crear un servidor MCP simple en Python que busca información de libros, vinculándose al proyecto de la aplicación de libros que has estado usando a lo largo de este curso.
...
> 💡 **¿Qué es el paquete `mcp`?** Es el SDK oficial de Python para construir servidores MCP. Gestiona los detalles del protocolo para que puedas concentrarte en tus herramientas.
...
# Base de datos de ejemplo de libros (en un servidor real, esto podría consultar una API o una base de datos)
BOOKS_DB = {
    "978-0-547-92822-7": {
        "title": "The Hobbit",
...
> 💡 **¿Por qué decoradores?** El decorador `@mcp.tool()` es todo lo que necesitas. El SDK de MCP lee automáticamente el nombre de tu función, las sugerencias de tipo y el docstring para generar el esquema de la herramienta. ¡No se necesita un esquema JSON manual!
...
1. **Agregar más herramientas** - Cada `@mcp.tool()` función se convierte en una herramienta que Copilot puede invocar
2. **Conectar APIs reales** - Reemplaza la `BOOKS_DB` simulada con llamadas API reales o consultas a bases de datos
