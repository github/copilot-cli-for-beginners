...
¿Quieres conectar Copilot a tus propias APIs? Aquí se muestra cómo crear un servidor MCP sencillo en Python que busca información de libros, vinculándolo con el proyecto de aplicación de libros que has estado usando a lo largo de este curso.
...
| `@mcp.tool()` | Registra una función como una herramienta que Copilot puede llamar |
| Type hints + docstrings | Le indican a Copilot qué hace cada herramienta y qué parámetros necesita |
...
2. **Conectar APIs reales** - Reemplaza la base de datos simulada `BOOKS_DB` por llamadas a APIs reales o consultas a la base de datos
3. **Agregar autenticación** - Gestionar claves de API y tokens de forma segura
4. **Compartir tu servidor** - Publicarlo en PyPI para que otros puedan instalarlo con `pip`
