...
---
id: CopilotCLI-03
title: !translate Flujos de trabajo de desarrollo
description: !translate Aplicar GitHub Copilot CLI a los flujos de trabajo de desarrollo cotidianos, incluyendo revisión de código, refactorización, depuración, generación de pruebas y Git.
...
En este capítulo, GitHub Copilot CLI se convierte en tu herramienta principal. Lo usarás dentro de los flujos de trabajo en los que ya confías cada día: pruebas, refactorización, depuración y Git.
...
- Depurar problemas con ayuda de la IA
- Generar pruebas automáticamente
- Integrar Copilot CLI en tu flujo de trabajo de Git
...
<img src="../../../03-development-workflows/assets/carpenter-workflow-steps.png" alt="Taller de artesano que muestra tres carriles de flujo de trabajo: Construcción de muebles (Medir, Cortar, Ensamblar, Acabar), Reparación de daños (Evaluar, Retirar, Reparar, Emparejar), y Control de calidad (Inspeccionar, Probar Uniones, Verificar Alineación)" width="800"/>

De igual forma, los desarrolladores tienen flujos de trabajo para distintas tareas. GitHub Copilot CLI mejora cada uno de estos flujos, haciéndote más eficiente y eficaz en tus tareas diarias de programación.

---

# Los cinco flujos de trabajo

<img src="../../../03-development-workflows/assets/five-workflows.png" alt="Cinco iconos neón brillantes que representan los flujos de trabajo de revisión de código, pruebas, depuración, refactorización e integración con git" width="800"/>
...
Este capítulo cubre cinco flujos de trabajo que los desarrolladores suelen usar. **¡Sin embargo, no necesitas leerlos todos de una vez!** Cada flujo de trabajo está contenido en una sección plegable abajo. Elige los que coincidan con lo que necesitas y que mejor se ajusten a tu proyecto actual. Siempre puedes volver y explorar los demás más tarde.
...
| Limpiar código desordenado o legado | [Flujo de trabajo 2: Refactorización](#workflow-2-refactoring) |
| Localizar y arreglar un error | [Flujo de trabajo 3: Depuración](#workflow-3-debugging) |
...
| Investigar antes de codificar | [Consejo rápido: Investiga antes de planificar o codificar](#usar-delegate-para-tareas-en-segundo-plano) |
| Ver un flujo de corrección de errores completo de principio a fin | [Poniéndolo todo junto](#usar-diff-para-revisar-los-cambios-de-la-sesión) |
...
Pide a Copilot CLI que centre su revisión en una preocupación específica (aquí, la validación de entradas) enlistando las categorías que te importan en la solicitud.
...
Usa una conversación de múltiples turnos para profundizar. Comienza con una revisión general y luego haz preguntas de seguimiento sin reiniciar.
...
### Plantilla de lista de verificación para revisiones
...
|-------------|---------------|------------|
| **Cambios preparados (staged)** | Archivos que has marcado para el próximo commit con `git add` | `git diff --staged` |
| **Cambios no preparados (unstaged)** | Archivos que has modificado pero que todavía no has añadido | `git diff` |
...
El comando `/review` invoca al **agente de revisión de código** incorporado, que está optimizado para analizar cambios preparados y no preparados con una salida de alta relación señal/ruido. Usa un comando con barra para activar un agente incorporado especializado en lugar de escribir una solicitud libre.

```bash
copilot

> /review
# Invoca al agente de revisión de código sobre cambios indexados/no indexados
...
Comienza con mejoras sencillas. Prueba estas en la aplicación de libros. Cada solicitud usa una referencia de archivo `@` acompañada de una instrucción de refactorización específica para que Copilot CLI sepa exactamente qué cambiar.
...
> 💡 **¿Nuevo en refactorización?** Empieza con solicitudes sencillas como agregar anotaciones de tipo o mejorar nombres de variables antes de abordar transformaciones complejas.
...
# Refactoriza con confianza - las pruebas verifican que se preserva el comportamiento
```
...
> **Prueba esto primero:** `@samples/book-app-buggy/books_buggy.py Los usuarios informan que buscar "The Hobbit" no devuelve resultados aunque esté en los datos. Depura por qué.`

Comienza describiendo qué está mal. Aquí hay patrones comunes de depuración que puedes probar con la aplicación de libros con errores. Cada solicitud empareja una referencia de archivo `@` con una descripción clara del síntoma para que Copilot CLI pueda localizar y diagnosticar el error.
...
Aquí es donde brilla la depuración consciente del contexto. Prueba este escenario con la aplicación de libros con errores. Proporciona el archivo completo mediante `@` y describe solo el síntoma reportado por el usuario. Copilot CLI rastreará la causa raíz y puede detectar errores adicionales cercanos.
...
> 💡 **Extra**: Dado que Copilot CLI analiza todo el archivo, a menudo descubre *otros* problemas sobre los que no preguntaste. Por ejemplo, al arreglar la búsqueda por autor, Copilot CLI podría también notar el error de sensibilidad a mayúsculas/minúsculas en `find_book_by_title`!

### Recuadro de seguridad del mundo real

Aunque depurar tu propio código es importante, entender las vulnerabilidades de seguridad en aplicaciones en producción es crítico. Prueba este ejemplo: apunta Copilot CLI a un archivo desconocido y pídele que audite problemas de seguridad.
...
> - **Inyección SQL (SQL Injection)**: Cuando la entrada del usuario se inserta directamente en una consulta de base de datos, permitiendo a atacantes ejecutar comandos maliciosos
...
> - **Condición de carrera (Race condition)**: Cuando dos operaciones ocurren al mismo tiempo y se interfieren entre sí
...
Pega un stack trace directamente en tu solicitud junto con una referencia de archivo `@` para que Copilot CLI pueda relacionar el error con el código fuente.
...
Observa lo que ocurre cuando le pides a Copilot CLI que genere pruebas exhaustivas. ¡Esta solicitud usa una lista de viñetas estructurada con una referencia de archivo `@` para guiar a Copilot CLI hacia una cobertura de pruebas completa:
...
Pregunta a Copilot CLI en lenguaje sencillo sobre tu cadena de herramientas. Puede generar el comando de shell correcto para ti.
...
Enumera escenarios avanzados o complejos que quieras cubrir para que Copilot CLI vaya más allá del camino feliz.
...
> 💡 **Este flujo de trabajo asume una familiaridad básica con git** (preparación (staging), commits, ramas). Si git es nuevo para ti, prueba primero los otros cuatro flujos de trabajo.

### Generar mensajes de commit

> **Prueba esto primero:** `copilot -p "Generate a conventional commit message for: $(git diff --staged)"` — prepara algunos cambios (stage), luego ejecuta esto para ver a Copilot CLI redactar tu mensaje de commit.

Este ejemplo usa la bandera de aviso en línea `-p` con sustitución de comandos de shell para redirigir la salida de `git diff` directamente a Copilot CLI para un mensaje de commit de una sola vez. La sintaxis `$(...)` ejecuta el comando dentro de los paréntesis e inserta su salida en el comando externo.
...
# - Agregar comparación sin distinción entre mayúsculas y minúsculas
...
Pasa la salida de `git show` a un aviso `-p` para obtener un resumen en lenguaje sencillo del último commit.
...
Si trabajas con una rama en el modo interactivo de Copilot CLI, puedes usar el comando `/pr` para gestionar pull requests. Usa `/pr` para ver un PR, crear uno nuevo, corregir un PR existente o dejar que Copilot CLI decida automáticamente según el estado de la rama.
...
Usa `git diff main..HEAD` dentro de un aviso `-p` para una comprobación rápida previa al push de todos los cambios de la rama.
...
El comando `/diff` muestra todos los cambios realizados durante tu sesión actual. Usa este comando de barra para ver un diff visual de todo lo que Copilot CLI ha modificado antes de que confirmes. También funciona en carpetas que no son repositorios git.
...
A veces quieres explorar dos enfoques diferentes para un problema sin perder tu conversación original. El comando `/branch` (también disponible como `/fork`) crea una copia de tu sesión actual para que puedas probar otra dirección y luego comparar resultados.
...
# Ahora estás en una nueva copia de sesión. Prueba tu enfoque alternativo:
...
> 💡 **Cuándo ramificar**: Ramificar es ideal cuando no estás seguro de qué enfoque es mejor y quieres mantener ambas opciones abiertas.
...
Cuando necesites investigar una librería, comprender buenas prácticas o explorar un tema desconocido, usa `/research` para realizar una investigación profunda antes de escribir código:
...
Copilot busca en repositorios de GitHub y fuentes web, luego devuelve un resumen con referencias. Esto es útil cuando estás a punto de empezar una nueva funcionalidad y quieres tomar decisiones informadas primero. Puedes compartir los resultados usando `/share`.

> 💡 **Consejo**: `/research` funciona bien *antes* de `/plan`. Investiga el enfoque, luego planifica la implementación.

---

## Poniéndolo todo junto: flujo de trabajo para corrección de errores
...
# Salida de ejemplo: "fix(books): admitir búsqueda parcial por nombre de autor"
...
| 2 | Análisis y solución | `> Show me the function and fix the issue` |
...
1. **Desafío de detective de bugs**: Pide a Copilot CLI que depure la función `mark_as_read` en `samples/book-app-buggy/books_buggy.py`. ¿Explicó por qué la función marca TODOS los libros como leídos en lugar de solo uno?

2. **Desafío de pruebas**: Genera pruebas para la función `add_book` en la app de libros. Cuenta cuántos casos límite incluye Copilot CLI que no habrías pensado.

3. **Desafío de mensaje de commit**: Haz cualquier pequeño cambio en un archivo de la app de libros, prepáralo (`git add .`), luego ejecuta:
   ```bash
   copilot -p "Generate a conventional commit message for: $(git diff --staged)"
   ```
   ¿Es el mensaje mejor que lo que habrías escrito rápidamente?

**Autoevaluación**: Comprendes los flujos de trabajo de desarrollo cuando puedes explicar por qué "debug this bug" es más poderoso que "find bugs" (¡el contexto importa!).

---

## 📝 Tarea

### Desafío principal: Refactorizar, probar y desplegar

Los ejemplos prácticos se centraron en `find_book_by_title` y en revisiones de código. Ahora practica las mismas habilidades de flujo de trabajo en diferentes funciones en `book-app-project`:
...
3. **Probar**: Genera pruebas pytest específicamente para la función `remove_book()` mejorada, cubriendo:
...
4. **Revisión**: Prepara tus cambios y ejecuta `/review` para comprobar si hay problemas restantes
...
**Consejo:** Después de mejorar `remove_book()`, intenta preguntarle a Copilot CLI: "Are there any other functions in this file that could benefit from the same improvements?". Puede sugerir cambios similares en `find_book_by_title()` o `find_by_author()`.

</details>

### Desafío extra: Crea una aplicación con el Copilot CLI
...
El ejercicio muestra a los desarrolladores cómo usar GitHub Copilot CLI para crear issues, generar código y colaborar desde la terminal mientras construyen una aplicación de calculadora en Node.js. Instalarás la CLI, usarás plantillas y agentes, y practicarás desarrollo iterativo impulsado por la línea de comandos.

##### <img src="../../../assets/github-skills-logo.png" width="28" align="center" /> [Comenzar el ejercicio de Skills "Crear aplicaciones con el Copilot CLI"](https://github.com/skills/create-applications-with-the-copilot-cli)
...
|---------|--------------|-----|
| Usar prompts vagos como "Review this code" | Comentarios genéricos que omiten problemas específicos | Sé específico: "Review for SQL injection, XSS, and auth issues" |
| No usar `/review` para revisiones de código | Falta el agente de revisión de código optimizado | Usa `/review` que está ajustado para una alta relación señal-ruido |
| Pedir que "find bugs" sin contexto | Copilot CLI no sabe qué error estás experimentando | Describe el síntoma: "Users report X happens when Y" |
| Generar pruebas sin especificar el framework | Las pruebas pueden usar sintaxis incorrecta o una librería de aserciones distinta | Especifica: "Generate tests using Jest" o "using pytest" |
...
**El refactor cambia el comportamiento** - Pídele a Copilot CLI que preserve el comportamiento:
...
1. **Revisión de código** se vuelve más completa con prompts específicos
2. **Refactorización** es más segura cuando generas pruebas primero
3. **Depuración** se beneficia de mostrarle a Copilot CLI el error Y el código
...
|-------|---------|----------------|
| Comandos básicos | Ch 01 | Usar modo interactivo, modo plan, modo programático (-p) y comandos de barra |
...
- **Comienza con `/plan`** para cualquier cosa no trivial. Refina el plan antes de la ejecución: un buen plan conduce a mejores resultados.
...
- **Experimenta libremente.** Algunos desarrolladores prefieren prompts largos y detallados. Otros prefieren prompts cortos con seguimientos. Prueba diferentes enfoques y observa qué se siente natural.

> 💡 **Próximamente**: En los capítulos 04 y 05, aprenderás cómo codificar tus mejores prácticas en instrucciones y skills personalizadas que Copilot CLI carga automáticamente.
...
|---------|----------------|---------------------|
| Ch 04: Agentes | Crear personas de IA especializadas | Cuando quieras expertos de dominio (frontend, seguridad) |
