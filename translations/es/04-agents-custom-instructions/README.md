...
En el Capítulo 03, dominaste los flujos de trabajo esenciales: revisión de código, refactorización, depuración, generación de tests e integración con git. Eso te hace muy productivo con GitHub Copilot CLI. Ahora, vamos más allá.

Hasta ahora, has estado usando Copilot CLI como un asistente de propósito general. Los agentes te permiten darle una persona específica con estándares integrados, como un revisor de código que aplica anotaciones de tipo y PEP 8, o un ayudante de testing que escribe casos de pytest. Verás cómo el mismo prompt obtiene resultados notablemente mejores cuando lo maneja un agente con instrucciones dirigidas.
...
|---------:|--------------|--------|
| Leaky pipe | Plumber | Conoce los códigos de fontanería, tiene herramientas especializadas |
| Rewiring | Electrician | Entiende los requisitos de seguridad, cumple con el código |
| New roof | Roofer | Conoce los materiales y las consideraciones del clima local |

Los agentes funcionan de la misma manera. En lugar de una IA genérica, usa agentes que se enfocan en tareas específicas y conocen el proceso correcto a seguir. Configura las instrucciones una vez y luego reutilízalas siempre que necesites esa especialidad: revisión de código, pruebas, seguridad, documentación.

<img src="../../../04-agents-custom-instructions/assets/hiring-specialists-analogy.png" alt="Analogía de especialistas - Al igual que llamas a oficios especializados para reparar la casa, los agentes de IA están especializados en tareas específicas como revisión de código, pruebas, seguridad y documentación" width="800" />
...
¿Nunca has usado o creado un agente? Aquí tienes todo lo que necesitas saber para empezar en este curso.
...
2. **Ve uno de nuestros ejemplos de agentes personalizados:** Es sencillo definir las instrucciones de un agente; consulta nuestro archivo [python-reviewer.agent.md](../../../.github/agents/python-reviewer.agent.md) para ver el patrón.
...
<br>`/plan` and `/review` son en realidad agentes integrados. Ahora sabes qué sucede detrás de escena. Aquí está la lista completa:
...
| **Code-review** | `/review` | Revisa cambios staged/unstaged con comentarios concretos y accionables |
...
| **Task** | *Automatic* | Ejecuta comandos como pruebas, compilación, lint e instalación de dependencias |
...
¿Qué pasa con el agente Task? Trabaja entre bastidores para gestionar y seguir lo que está ocurriendo y para informar de vuelta en un formato limpio y claro:

| Resultado | Lo que ves |
|---------|-----------|
| ✅ **Success** | Resumen breve (p. ej., "All 247 tests passed", "Build succeeded") |
| ❌ **Failure** | Salida completa con traces de pila, errores del compilador y registros detallados |
...
¡Puedes definir tus propios agentes para integrarlos en tu flujo de trabajo! Define una vez y ¡dirige!

<img src="../../../04-agents-custom-instructions/assets/using-agents.png" alt="Cuatro robots coloridos de IA de pie juntos, cada uno con diferentes herramientas que representan capacidades especializadas de agentes" width="800"/>

## 🗂️ Añade tus agentes

Los archivos de agente son archivos markdown con la extensión `.agent.md`. Tienen dos partes: frontmatter YAML (metadatos) y instrucciones en markdown.

> 💡 **¿Nuevo en frontmatter YAML?** Es un pequeño bloque de configuración en la parte superior del archivo, rodeado por marcadores `---`. YAML son solo pares `clave: valor`. El resto del archivo es markdown normal.
...
### Modo interactivo
Dentro del modo interactivo, lista agentes usando `/agent` y selecciona el agente con el que quieres empezar a trabajar. 
...
Para cambiar a un agente diferente, o para volver al modo predeterminado, usa el comando `/agent` nuevamente.
...
> 💡 **Cambiar de agente**: Puedes cambiar a un agente diferente en cualquier momento usando `/agent` o `--agent` otra vez. Para volver a la experiencia estándar de Copilot CLI, usa `/agent` y selecciona **ningún agente**.

> 💡 **El modo agente es por sesión**: El agente que selecciones se aplica solo a la sesión actual. Cuando inicias una nueva sesión con `/new`, `/clear` o abres una terminal nueva, Copilot vuelve a su modo predeterminado: la selección de agente no se conserva automáticamente. Esto significa que cada sesión comienza con una pizarra limpia, lo cual es una buena práctica para mantener tu trabajo enfocado.
...
> 💡 **Esta sección es opcional.** Los agentes integrados (`/plan`, `/review`) son lo suficientemente potentes para la mayoría de los flujos de trabajo. Crea agentes personalizados cuando necesites experiencia especializada que se aplique de forma consistente a tu trabajo.
...
| Configurar contexto del proyecto siempre activo | [Configurar tu proyecto para Copilot](#configurando-su-proyecto-para-copilot) |
| Consultar propiedades YAML y herramientas | [Referencia de archivo de agente](#referencia-de-archivos-de-agente) |
...
<details>
<summary><strong>Especialista vs Genérico: Ve la diferencia</strong> - Por qué los agentes producen mejor salida que los prompts genéricos</summary>

## Especialista vs Genérico: Ve la diferencia

Aquí es donde los agentes demuestran su valor. Observa la diferencia:

### Sin agente (Copilot genérico)
...
- ✅ Docstring comprensivo con Args/Returns/Raises
...
El verdadero poder surge cuando los especialistas colaboran en una funcionalidad.
...
Ya conoces las dos ubicaciones principales (ver [Dónde colocar los archivos de agente](#dónde-colocar-los-archivos-de-agente) más arriba). Usa este árbol de decisiones para elegir:


<img src="../../../04-agents-custom-instructions/assets/agent-file-placement-decision-tree.png" alt="Árbol de decisiones para dónde colocar archivos de agentes: experimentación → carpeta actual, uso en equipo → .github/agents/, en todas partes → ~/.copilot/agents/" width="800"/>

**Empieza simple:** Crea un único archivo `*.agent.md` en la carpeta de tu proyecto. Muévelo a una ubicación permanente una vez que estés satisfecho.

Más allá de los archivos de agentes, Copilot también lee automáticamente los **archivos de instrucciones a nivel de proyecto**, no se necesita `/agent`. Consulta [Configuring Your Project for Copilot](#configurando-su-proyecto-para-copilot) más abajo para `AGENTS.md`, `.instructions.md` y `/init`.
...
<details>
<summary><strong>Configurando su proyecto para Copilot</strong> - AGENTS.md, archivos de instrucciones y configuración /init</summary>
...
### Formatos de archivos de instrucción

| File | Scope | Notes |
|------|-------|-------|
| `AGENTS.md` | Project root or nested | **Estándar multiplataforma** - funciona con Copilot y otros asistentes de IA |
...
| `~/.copilot/instructions/**/*.instructions.md` | User (all projects) | Instrucciones personales que se aplican en todas partes, en todos tus repositorios |
| `CLAUDE.md`, `GEMINI.md` | Project root | Compatibles para interoperabilidad |

> 🎯 **¿Acabas de empezar?** Usa `AGENTS.md` para instrucciones del proyecto. Puedes explorar los otros formatos más adelante según sea necesario.
...
Para equipos que desean un control más granular, separa las instrucciones en archivos por tema. Cada archivo cubre una preocupación y se aplica automáticamente:
...
#### Restringir el alcance de las instrucciones con `applyTo`

Por defecto, un archivo de instrucciones se aplica a toda conversación. Para limitarlo a tipos de archivo específicos, añade un campo `applyTo` en el frontmatter YAML (el bloque entre marcadores `---` al principio del archivo):
...
Con `applyTo: "**/*.py"`, Copilot solo carga ese archivo de instrucciones cuando estás trabajando con archivos Python. Las instrucciones de estilo para Python no entorpecerán una conversación sobre, por ejemplo, un Dockerfile o una consulta SQL.
...
| (no frontmatter) | Toda conversación — por defecto |

> 💡 **Consejo**: Envuelve el patrón glob en comillas (por ejemplo, `"**/*.py"`) para asegurarte de que se interprete correctamente en todos los sistemas operativos y shells.

**Encontrar archivos de instrucciones de la comunidad**: Explora [github/awesome-copilot](https://github.com/github/awesome-copilot) para archivos de instrucciones ya preparados que cubren .NET, Angular, Azure, Python, Docker y muchas más tecnologías.
...
Has visto el [formato mínimo de agente](#-add-your-agents) más arriba. Aquí tienes un agente más completo que usa la propiedad `tools`. Crea `~/.copilot/agents/python-reviewer.agent.md`:
...
|----------|----------|-------------|
| `name` | No | Nombre a mostrar (por defecto, el nombre del archivo) |
| `description` | **Yes** | Qué hace el agente - ayuda a Copilot a entender cuándo sugerirlo |
...
>
> ⚠️ **Solo VS Code**: La propiedad `model` (para seleccionar modelos de IA) funciona en VS Code pero no está soportada en GitHub Copilot CLI. Puedes incluirla de forma segura para archivos de agente multiplataforma. GitHub Copilot CLI la ignorará.

### Más plantillas de agentes

> 💡 **Nota para principiantes**: Los ejemplos siguientes son plantillas. **Sustituye las tecnologías específicas por las que use tu proyecto.** Lo importante es la *estructura* del agente, no las tecnologías específicas mencionadas.

Este proyecto incluye ejemplos funcionales en la carpeta [.github/agents/](../../../.github/agents):
- [hello-world.agent.md](../../../.github/agents/hello-world.agent.md) - Ejemplo mínimo, empieza aquí
...
Crea tus propios agentes y vívelos en acción.
...
2. Tus agentes:
   - **data-validator**: revisa `data.json` en busca de datos faltantes o malformados (autores vacíos, year=0, campos faltantes)
...
- `python-style.instructions.md` para aplicar PEP 8 y convenciones de hints de tipo
...
|---------|--------------|-----|
| Missing `description` in agent frontmatter | Agent won't load or won't be discoverable | Incluye siempre `description:` en el frontmatter YAML |
| Wrong file location for agents | Agent not found when you try to use it | Colócalos en `~/.copilot/agents/` (personal) o `.github/agents/` (proyecto) |
| Using `.md` instead of `.agent.md` | File may not be recognized as an agent | Nombra los archivos como `python-reviewer.agent.md` |
| Overly long agent prompts | May hit the 30,000 character limit | Mantén las definiciones de agente enfocadas; usa skills para instrucciones detalladas |

### Solución de problemas

**Agent not found** - Verifica que el archivo de agente exista en una de estas ubicaciones:
- `~/.copilot/agents/`
- `.github/agents/`
...
**Agent not following instructions** - Sé explícito en tus prompts y añade más detalle a las definiciones de agente:
...
4. **Colaboración multi-agente** resuelve problemas complejos combinando pericias
5. **Archivos de instrucciones** (`.instructions.md`) codifican los estándares del equipo para aplicación automática
...
Los agentes cambian *cómo Copilot enfoca y realiza acciones dirigidas* en tu código. A continuación, aprenderás sobre **skills** - que cambian *qué pasos* sigue. ¿Te preguntas cómo difieren agentes y skills? El Capítulo 05 lo cubre a fondo.

En **[Chapter 05: Skills System](../05-skills/README.md)**, aprenderás:

- Cómo las skills se activan automáticamente a partir de tus prompts (sin necesidad de comando slash)
